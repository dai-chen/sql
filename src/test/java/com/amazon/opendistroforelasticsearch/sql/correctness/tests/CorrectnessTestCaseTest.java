...
import com.amazon.opendistroforelasticsearch.sql.correctness.CorrectnessTestCase;
...
public class CorrectnessTestCaseTest {
    ...
    private CorrectnessTestCase correctnessTest;

    @Before
    public void setUp() {
        correctnessTest = new CorrectnessTestCase(
    ...
    public void testFailureDueToInconsistency() {
        ...
        TestReport expected = new TestReport();
        expected.addTestCase(new FailedTestCase("SELECT * FROM accounts", Arrays.asList(esResult, otherDbResult)));
        TestReport actual = correctnessTest.verify(asList("SELECT * FROM accounts"));
        assertEquals(expected, actual);
    }
