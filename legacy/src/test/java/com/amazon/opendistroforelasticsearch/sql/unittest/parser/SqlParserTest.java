...
public class SqlParserTest {
    ...
    private final static String TEST_INDEX = "elasticsearch-sql_test_index";
    private final static String TEST_INDEX_ACCOUNT = TEST_INDEX + "_account";
    private final static String TEST_INDEX_DOG = TEST_INDEX + "_dog";
    private final static String TEST_INDEX_GAME_OF_THRONES = TEST_INDEX + "_game_of_thrones";
    private final static String TEST_INDEX_ODBC = TEST_INDEX + "_odbc";
    ...
    public void innerQueryTestTwoQueries() throws SqlParseException {
        String query = String.format(Locale.ROOT, "select * from %s/dog where holdersName IN " +
                ...
                TEST_INDEX_DOG, TEST_INDEX_ACCOUNT, TEST_INDEX_GAME_OF_THRONES);
        SQLExpr sqlExpr = queryToExpr(query);
        Select select = parser.parseSelect((SQLQueryExpr) sqlExpr);
        Assert.assertTrue(select.containsSubQueries());
