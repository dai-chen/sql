...
import com.amazon.opendistroforelasticsearch.sql.executor.explain.Profiler.Profile;
...
public class Explain extends PhysicalPlanNodeVisitor<JsonNode, ObjectNode>
  ...
  private JsonNode explain(PhysicalPlan node, ObjectNode parent,
    ...
    Profile profile = profiler.find(node);
    if (profile != null) {
      json.set("profile",
                mapper.createObjectNode()
                      .put("rows", profile.getRows())
                      .put("elapsed (ms)", profile.getElapsed()));
    }
