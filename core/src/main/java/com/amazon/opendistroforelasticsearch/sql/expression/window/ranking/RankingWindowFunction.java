...
import com.amazon.opendistroforelasticsearch.sql.expression.window.WindowFunction;
 ...
 */
public abstract class RankingWindowFunction extends FunctionExpression implements WindowFunction {
  ...
  protected boolean isSortFieldValueDifferent(CumulativeWindowFrame frame) {
    ...
    List<ExprValue> previous = resolve(sortItems, frame.previous());
    List<ExprValue> current = resolve(sortItems, frame.current());
